...
const handleCreateDraft = async () => {
  ...
  try {
    const draft = await createTravelReimburseDraft()
    openDraftDetail(draft.id, draft.redirectUrl)
  } catch (error) {
    console.warn(error)
    const localId = uid('draft')
    openDraftDetail(localId, `reimburse/detail/${localId}`)
    ElMessage.warning('草稿接口未连接，已打开模板详情')
  }
}
...
const openDraftDetail = (id: string, redirectUrl: string) => {
  ...
  ElMessage.success(`已创建草稿：${redirectUrl}`)
}
...
const handleSaveDraft = async () => {
  ...
  try {
    ...
    console.warn(error)
    localDetailMap.value[detail.id] = detail
    upsertLocalRow(buildRowFromDetail(detail))
    ElMessage.warning('接口未连接，已在本地保存草稿')
  } finally {
    saving.value = false
  }
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const submittedDetail: TravelReimburseDetail = {
      ...detail,
      reimBillNo: detail.reimBillNo || `CLBX${new Date().toISOString().slice(0, 10).replace(/-/g, '')}${String(Date.now()).slice(-4)}`,
      billStatus: '1',
      billStatusName: '已完成'
    }
    localDetailMap.value[submittedDetail.id] = submittedDetail
    ...
    applyDetailToForm(submittedDetail)
    detailMode.value = 'view'
    ElMessage.warning('接口未连接，已在本地模拟提交')
  } finally {
    saving.value = false
  }
}
